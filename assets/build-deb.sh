...
if [[ ! -f /tmp/nghttp2.deb || "$(dpkg-deb --field /tmp/nghttp2.deb version)" != "$nghttp2_v-1" ]]; then
	...
	autoreconf -i && automake && autoconf && ./configure && make install
	...
	checkinstall -y --requires="libevent-openssl-2.0-5, libevent-2.0-5, libjemalloc-dev, libev-dev" --install=no --maintainer=tatsuhiro-t

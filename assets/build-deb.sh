...
nghttp2_v=$(curl https://github.com/tatsuhiro-t/nghttp2/releases | grep -o '[0-9]\.[0-9]\.[0-9]' | head -1) 
if [[ ! -f /tmp/nghttp2.deb || "$(dpkg-deb --field /tmp/nghttp2.deb version)" != "$nghttp2_v-1" ]]; then
  apt-get -y install libjansson-dev libjemalloc-dev cython python3.4-dev
  cd /tmp && wget --no-check-certificate https://github.com/tatsuhiro-t/nghttp2/releases/download/v$nghttp2_v/nghttp2-$nghttp2_v.tar.gz
  ...
  cp /tmp/nghttp2-0.4.1/nghttp2_$nghttp2_v-1_amd64.deb /tmp/nghttp2.deb
fi
